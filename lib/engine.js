...
$("#genBtn").addEventListener("click", async () => {
  ...
  const want = S.kind === "auto"
    ? "Elige tú el formato entre: texto, tabla, comparativo, mapa, proceso."
  ...
  const prompt =
    ...
    "FORMATO: <texto|tabla|comparativo|mapa|proceso>\n" +
    "TITULO: <título corto>\n" +
...
$("#polish").addEventListener("click", async () => {
  ...
  if (!sampleFn){
    ...
    istatus("Pulido aquí mismo: ortografía, tildes y mayúsculas.");
    return;
  }
  const b = $("#polish"); b.disabled = true; istatus("Reescribiendo…");
  const prompt =
    "Reescribe este paso de un instructivo de bodega en español de Colombia.\n" +
    "Corrige ortografía y tildes. Déjalo claro y corto, en imperativo. No inventes datos ni pasos nuevos. " +
    "Conserva igual los códigos y transacciones (SAP, ZMEB, VL02N, VT02N, VF01, Ag01, Ag22).\n" +
    "Responde EXACTAMENTE en dos líneas, sin nada más:\n" +
    "TITULO: <3 a 6 palabras en mayúsculas>\n" +
    "TEXTO: <1 o 2 frases>\n\n" +
    "Título actual: " + (tit || "(vacío)") + "\nPaso escrito: " + (raw || "(vacío)").slice(0, 1200);
  try{
    const r = await sampleFn(prompt, {modelTier:"quick", cache:false});
    const out = leerMarcas(r.text);
    if (out.text || out.title){
      if (out.title) $("#stepTitle").value = out.title;
      if (out.text) $("#stepText").value = out.text;
      istatus(editStep ? "Listo. Toca Guardar cambios." : "Listo. Revisa y agrega el paso.");
    } else istatus("No entendí la respuesta. Vuelve a intentar.", true);
