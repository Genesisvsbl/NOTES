 ...
 * Framework-agnóstico: se monta en un contenedor del DOM.
 *   mountNotesGene(root, { store, ai })
 */
...
let bgImg = null, sampleFn = null, sampleImages = false;
...
/* ---- texto manuscrito/tecleado ---- */
let tbox = null;
function openText(x, y){
  closeText();
  const size = 20 + S.size*8;
  tbox = document.createElement("textarea");
  tbox.id = "txtbox"; tbox.rows = 1;
  tbox.style.left = (x*S.zoom)+"px"; tbox.style.top = (y*S.zoom)+"px";
  tbox.style.fontSize = (size*S.zoom)+"px"; tbox.style.color = S.color;
  tbox.style.fontFamily = "'"+S.font+"', sans-serif";
  tbox.dataset.x = x; tbox.dataset.y = y; tbox.dataset.size = size;
  $("#sheet").appendChild(tbox);
  setTimeout(() => tbox && tbox.focus(), 10);
  ...
  tbox.addEventListener("keydown", ev => { if (ev.key === "Escape"){ tbox.value = ""; tbox.blur(); } });
}
function closeText(){
  if (!tbox) return;
  const v = tbox.value.trim(), b = tbox; tbox = null;
  if (v){
    pushUndo();
    S.items.push({t:"t", c:b.style.color, w:+b.dataset.size, x:+b.dataset.x, y:+b.dataset.y, txt:v, f:S.font});
    redraw(); markDirty();
  }
  b.remove();
}
...
function setZoom(z){
  S.zoom = Math.max(.35, Math.min(3, z));
...
$("#studioClose").addEventListener("click", () => { $("#studio").classList.add("hide"); stopRec(); });
...
const ERRS = {
  not_granted:"No autorizaste que esta página use Claude.",
...
function errMsg(e){ return (e && ERRS[e.code]) || "No se pudo completar ("+((e&&e.code)||"error")+")."; }
...
$("#insBtn").addEventListener("click", () => {
  ...
  S.items.push(obj);
  redraw(); markDirty();
  setStatus("Insertado en la página "+(S.pi+1)+". Con la herramienta Mover puedes acomodarlo.");
...
$("#instrClose").addEventListener("click", () => {
  $("#instr").classList.add("hide"); $("#instrBtn").classList.remove("on"); stopRec();
});
...
$("#stepPhotoX").addEventListener("click", () => {
  stepPhoto = null; $("#stepPrev").classList.add("hide"); istatus("");
});

$("#polish").addEventListener("click", async () => {
  const raw = $("#stepText").value.trim();
  if (!raw){ istatus("Primero dicta o escribe el paso.", true); return; }
  if (!sampleFn){ istatus("La IA no está disponible en esta vista.", true); return; }
  const b = $("#polish"); b.disabled = true; istatus("Puliendo…");
  try{
    const r = await sampleFn.json(
      "Reescribe este paso de un instructivo de trabajo en español (Colombia) como instrucción clara y corta, " +
      "en imperativo, sin inventar datos ni agregar pasos. Devuelve SOLO un JSON " +
      '{"title":"título de 3 a 6 palabras","text":"descripción de 1 a 3 frases"}.\n\nPASO DICTADO:\n' + raw.slice(0, 3000),
      {modelTier:"quick", cache:false});
    if (r && (r.text || r.title)){
      if (r.title && !$("#stepTitle").value.trim()) $("#stepTitle").value = r.title;
      if (r.text) $("#stepText").value = r.text;
      istatus("Listo. Revisa y agrega el paso.");
...
async function delStep(pIdx, it){
  if (!confirm("¿Eliminar el paso "+(it.n||"")+"?")) return;
...
$("#addStep").addEventListener("click", async () => {
  ...
  const b = $("#addStep"); b.disabled = true; istatus("Agregando…");
  try{
    ...
    stepPhoto = null; $("#stepPrev").classList.add("hide");
    await renderSteps();
...
async function menuAction(a){
  ...
  if (a === "clear"){
    ...
    if (!confirm("¿Borrar todo el contenido de esta página?")) return;
...
async function refreshLibrary(){
  S.notebooks = await Store.listNotebooks();
  ...
  S.notebooks.forEach(nb => {
    ...
    card.querySelector('[data-a="ren"]').addEventListener("click", async () => {
      const n = prompt("Nombre del cuaderno:", nb.name); if (n == null) return;
    ...
    card.querySelector('[data-a="del"]').addEventListener("click", async () => {
      if (!confirm('¿Eliminar "'+nb.name+'" con todas sus páginas?')) return;
...
async function createNotebook(){
  const name = prompt("Nombre del cuaderno:", "Cuaderno "+(S.notebooks.length+1));
  if (name == null) return;
...
async function openNotebook(nb){
  ...
  const fit = ($("#scroll").clientWidth - 60)/PW;
  setZoom(Math.max(.45, Math.min(1, fit)));
  await loadPage(0);
}
...
(async function start(){
  ...
  await refreshLibrary();
  $("#storeNote").innerHTML =
    '<span class="dot"></span><span>Guardado en tu cuenta de NotesGene: los cuadernos te siguen del iPad al computador.</span>';
