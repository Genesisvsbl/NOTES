...
$("#polish").addEventListener("click", async () => {
  ...
  const prompt =
    "Reescribe este paso de un instructivo de bodega en español de Colombia: ortografía y tildes correctas, " +
    "instrucción clara y corta en imperativo. No inventes datos ni pasos. Conserva igual los códigos y " +
    "transacciones (SAP, ZMEB, VL02N, VT02N, VF01, Ag01, Ag22).\n" +
    'Devuelve SOLO este JSON: {"title":"3 a 6 palabras EN MAYÚSCULAS","text":"1 a 2 frases"}\n\n' +
    "TÍTULO: " + (tit || "(vacío)") + "\nPASO: " + (raw || "(vacío)").slice(0, 1200);
  try{
    const r = await sampleFn.json(prompt, {modelTier:"quick", cache:false});
    if (r && (r.text || r.title)){
