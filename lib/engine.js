...
async function pdfDeCuaderno(nb){
  ...
  toast('Armando el PDF de "' + nb.name + '" (' + pages.length + (pages.length === 1 ? " página" : " páginas") + ')…', 6000);
  const {jsPDF} = window.jspdf;
  const pt0 = HOJA_PT[pages[0].hoja || "a4-v"];
  ...
  try{
    ...
    await offer(safeName(nb.name)+".pdf", doc.output("blob"));
