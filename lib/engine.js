...
function buildKinds(){
  ...
  KINDS.forEach(([id, label]) => {
    ...
    b.addEventListener("click", () => {
      S.kind = id; buildKinds(); $("#syntax").textContent = SYNTAX[id];
      /* lo que dictaste o escribiste arriba pasa al recuadro para editarlo y insertarlo */
      if (!$("#struct").value.trim()){
  ...
  $("#struct").placeholder = SYNTAX[S.kind];
}
...
$("#insBtn").addEventListener("click", () => {
  let raw = $("#struct").value.trim();
  /* si no llenaste «Estructura», vale lo que escribiste arriba: la IA no hace falta */
  if (!raw){ raw = $("#transcript").value.trim(); if (raw) $("#struct").value = raw; }
  if (!raw){ setStatus("Escribe las líneas aquí abajo y dale Insertar.", true); return; }
